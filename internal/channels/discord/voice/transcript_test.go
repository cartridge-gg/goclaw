...
func Test_processUtterance_allowsExpectedTranscriptLanguages(t *testing.T) {
	fs := &fakeSession{
		guildMemberFn: func(_, _ string) (*discordgo.Member, error) {
			return &discordgo.Member{Nick: "Alice"}, nil
		},
	...
	stt := &recordingSTTProvider{
		name:   "elevenlabs",
		result: &audio.TranscriptResult{Text: "bonjour tout le monde", Language: "fr", Provider: "elevenlabs"},
	}
	...
	if stt.opts.Language != "" {
		t.Fatalf("STT language hint = %q, want auto-detect", stt.opts.Language)
	}
...
func Test_processUtterance_dropsUnexpectedLanguageDetection(t *testing.T) {
	fs := &fakeSession{
		guildMemberFn: func(_, _ string) (*discordgo.Member, error) {
			return &discordgo.Member{Nick: "Alice"}, nil
		},
	...
	if got := fs.sendsByChannel["thread-1"]; len(got) != 0 {
		t.Fatalf("expected unexpected-language transcript to be dropped, got %v", got)
	}
}
