...
var discordAllowedTranscriptLanguages = map[string]struct{}{
	"en":  {},
	"eng": {},
	"fr":  {},
	"fra": {},
...
func (t *transcriber) processUtterance(ctx context.Context, u utterance) {
	...
	if !isAllowedTranscriptLanguage(result.Language) {
		t.log.Warn("voice: drop transcript with unexpected language detection",
...
func isAllowedTranscriptLanguage(language string) bool {
	language = strings.ToLower(strings.TrimSpace(language))
	if language == "" {
		return true
	}
	if _, ok := discordAllowedTranscriptLanguages[language]; ok {
		return true
	}
	for allowed := range discordAllowedTranscriptLanguages {
		if strings.HasPrefix(language, allowed+"-") || strings.HasPrefix(language, allowed+"_") {
			return true
		}
	}
	return false
